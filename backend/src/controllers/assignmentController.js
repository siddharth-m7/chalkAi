...
import { buildAssignmentSystemPrompt, buildAssignmentUserPrompt } from '../utils/prompts/assignmentPrompt.js'
...
export const getAssignment = async (req, res, next) => {
  try {
    ...
    if (!content) return res.status(404).json({ success: false, message: 'Assignment not found' })
...
export const listAssignments = async (req, res, next) => {
  try {
    const assignments = await GeneratedContent.find({
      userId: req.user._id,
      type: 'assignment'
    })
      .sort({ createdAt: -1 })
